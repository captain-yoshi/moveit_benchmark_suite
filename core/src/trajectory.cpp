...
void Trajectory::useMessage(const robot_state::RobotState& reference_state, const moveit_msgs::RobotTrajectory& msg)
{
  trajectory_->setRobotTrajectoryMsg(reference_state, msg);
}

void Trajectory::useMessage(const robot_state::RobotState& reference_state, const trajectory_msgs::JointTrajectory& msg)
{
  trajectory_->setRobotTrajectoryMsg(reference_state, msg);
}
...
bool Trajectory::computeTimeParameterization(robot_trajectory::RobotTrajectory& trajectory, double max_velocity,
...
void Trajectory::interpolate(unsigned int count)
{
  ...
  ROS_INFO_STREAM(log::format("Added %d extra states in the trajectory", added));
  return;
}
...
bool Trajectory::isCollisionFree(const planning_scene::PlanningSceneConstPtr& scene) const
{
  bool correct = true;
  ...
  for (std::size_t k = 0; k < trajectory_->getWayPointCount(); ++k)
  {
    ...
    scene->checkCollision(request, result, *s);
...
std::tuple<double, double, double> Trajectory::getClearance(const planning_scene::PlanningSceneConstPtr& scene) const
{
...
double Trajectory::getSmoothness(const PathMetric& metric) const
{
  ...
  auto distance = (metric) ? metric : [](const robot_state::RobotState& a, const robot_state::RobotState& b) {
    return a.distance(b);
  };
